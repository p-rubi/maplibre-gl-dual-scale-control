// Dual-scale barscale control adapted for Maplibre
// Minimal standalone implementation (CommonJS)
...
function updateScale(map, metricContainer, imperialContainer, options) {
  ...
  const a = map._container.clientHeight / 2;
  const r = getDistance(map.unproject([0, a]), map.unproject([maxWidth, a]));
  const s = 3.2808 * r;
  if (s > 5280) {
...
function getDistance(latlng1, latlng2) {
  ...
  const a = Math.sin(lat1) * Math.sin(lat2) + Math.cos(lat1) * Math.cos(lat2) * Math.cos((latlng2.lng - latlng1.lng) * rad);
...
DualScaleControl.prototype.getDefaultPosition = function () {
  return 'bottom-left';
};
...
if (typeof module !== 'undefined') module.exports = DualScaleControl;
